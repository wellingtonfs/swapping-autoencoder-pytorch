...
class CondPoseEncoder(BaseNetwork):
    def __init__(self, opt):
        ...
        self.add_module("fc_u", nn.Linear(in_features=512*1*1, out_features=self.opt.global_code_ch//2))
        self.add_module("fc_var", nn.Linear(in_features=self.opt.global_code_ch//2, out_features=self.opt.global_code_ch))
...
class CondPoseSPEncoder(BaseNetwork): #SP = Sem Pose
    def __init__(self, opt):
        super().__init__(opt)
        #(W−F+2P)/S + 1
        self.opt = opt
        ...
        self.add_module(
            ...
            nn.Sequential(
                #Show(),
                nn.Conv2d(in_channels=3, out_channels=128, kernel_size=7, stride=2, padding=1), #128x126x126
                nn.LeakyReLU(0.01),
                #Show(),
                nn.Conv2d(in_channels=128, out_channels=256, kernel_size=3, stride=2, padding=1), #256x63x63
                nn.ReLU(),
